...
headers_vision = {'Ocp-Apim-Subscription-Key': 'XXXXXXXXXXXXXXXXXXXXXXXXXXX'}
...
def analyse_tile(tile, gamestate):
    ...
    if subject is None:
        ...
        subject = check_for_landmark(msapi_response) #added gamestate landmarks
        
    if subject is None:
        subject = check_for_words(msapi_response) #added check_for_words
...
def check_for_words(msapi_response):
    ...
    for category in msapi_response["categories"]:
        # If the tag has a name and that name is one of the animals in our list
        if "detail" in category \
            and "Words" in category ["detail"] \
            and category["detail"]["Words"]:
        # Record the name of the animal that is the subject of the tile
                # (We store the subject in lowercase to make comparisons easier)
                subject = category["detail"]["Words"][0]["name"].lower()
                # Print out the animal we have found here for debugging ----------------->
                print("  Words: {}".format(subject))
                # Exit the for loop
